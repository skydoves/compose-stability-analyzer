...
- **The report directory no longer breaks Gradle build cache relocatability.** The output directory was passed to the compiler as a plain `SubpluginOption`, and KGP exposes every plain option through `CompilerPluginConfig.getAsTaskInputArgs()`, which is annotated `@Input`. The absolute checkout path therefore became part of the build cache key of every Kotlin compilation in a consuming module, so remote cache entries never matched between machines or worktrees and `compile*Kotlin` was never reused. It is now a `FilesSubpluginOption`, which KGP excludes from those input args while still passing the path to the compiler. Measured across two checkouts sharing one cache: a miss before, `FROM-CACHE` after, with the report restored. The report directory is also declared as a task output for the JS and metadata compilations, not only the JVM one, because they became relocatable at the same time and would otherwise hit the cache with nothing restoring their report. Kotlin/Native is unaffected by the fix: `KotlinNativeCompile` exposes plugin options as a plain `@Input` command line that no option subtype can keep out of the cache key. (#212)
- **`unstableOnly` now says so when it needs `ignoreNonRegressiveChanges`.** The filtered entries are absent from the baseline, and without that option `stabilityCheck` reports every missing composable as a new one, which fails the build just as permanently. `stabilityDump` now warns, naming the option and the number of entries it dropped, instead of leaving the combination to fail silently.
